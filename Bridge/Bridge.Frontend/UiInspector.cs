...
namespace Bridge.Frontend
{
    ...
    internal static class UiInspector
    {
        ...
        public static Dictionary<string, object> Snapshot()
        {
            ...
            if (cur != null)
            {
                foreach (var elem in cur)
                {
                    ...
                    if (root != null) CountTree(root, ref controls, ref texts, 3000);
        ...
        public static Dictionary<string, object> Element(string name, Options opts)
        {
            ...
            bool truncated = Collect(root, "", false, controls, texts, opts);

            var sections = GroupIntoSections(controls, texts);
        ...
        // ---------- 跨窗口文本搜索 ----------
        public static Dictionary<string, object> Find(string query, int limit)
        {
            UiElementCatalog.EnsureBuilt();
            ...
            if (cur != null)
            {
                foreach (var elem in cur)
                {
                    ...
                    var controls = new List<object>();
                    var texts = new List<object>();
                    Collect(root, "", false, controls, texts, new Options { Full = true, Max = 100000 });
            ...
            return new Dictionary<string, object> { ["query"] = query, ["count"] = hits.Count, ["hits"] = hits };
        }

        // ---------- 分区 ----------
        private static List<object> GroupIntoSections(List<object> controls, List<object> texts)
        {
        ...
        private static bool Collect(Transform t, string path, bool underControl,
            List<object> controls, List<object> texts, Options opts)
        {
            ...
            for (int i = 0; i < t.childCount; i++)
            {
                ...
                if (Collect(child, childPath, nowUnderControl, controls, texts, opts))
                    return true;
            }
            return false;
        }

        private static void CountTree(Transform t, ref int controls, ref int texts, int budget)
        {
            if (budget <= 0) return;
            var go = t.gameObject;
            var ctrl = UiTree.DetectControl(go);
            ...
            for (int i = 0; i < t.childCount; i++)
            {
                var child = t.GetChild(i);
                if (!child.gameObject.activeSelf) continue;
                CountTree(child, ref controls, ref texts, budget - 1);
            }
        }
