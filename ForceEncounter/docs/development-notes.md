...
- 行为者恒为太吾。后端不接受任何外部 `ActorId`（已移除该前向兼容参数），一律 `DomainManager.Taiwu.GetTaiwuCharId()`。
...
- 多个存活爱慕对象表示目标不是专一所属。
- 已死亡的爱慕对象不算当前所属。
- 亲密直通判定（当前实现，1.2.0 起）：满足任一亲密来源（配偶 / 双向爱慕 / 目标单恋太吾 / 谷中密友 / 太吾村民），且**目标对太吾好感 ≥ 热忱（`Favorite2 + 1`，即 `type >= 3`）**，即可走亲密。
- 只看目标对太吾一侧的好感（不看太吾对目标的好感），门槛统一为「热忱」、不按性格浮动；不再有村民宽免、非恋人下限或谷中密友「已有他属」特殊门槛。
...
- `谷中密友` 特性 `685` 只对原生谷中密友太吾 id 视为亲密来源；是否「已有他属」不再影响能否亲密，只影响好感惩罚折扣（见下）。
- 「已有他人所属」（定义见上）现在仅用于亲密成功后的好感惩罚判定，不参与亲密门槛。（`Language_CN/ui_language.txt` 将 `LK_Favor_Type_8` 映射为 `融洽`、`LK_Favor_Type_9` 为 `热忱`。）
